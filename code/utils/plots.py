...
def get_fig_threed_normals(normals):
    ...
    trace = [go.Cone(
                        ...
                        showscale=False,
                        showlegend=True,
                        visible=True) for p,n,name in normals]
    
    layout = go.Layout(width=800, height=800, scene=dict(xaxis=dict(range=[-2, 2], autorange=False,showbackground=False),
                                                           yaxis=dict(range=[-2, 2], autorange=False,showbackground=False),
                                                           zaxis=dict(range=[-2, 2], autorange=False,showbackground=False),
...
def get_fig_threed_scatter(points):
...
def plot_surface(with_points, points, decoder, latent, path, epoch, in_epoch,
    ...
    if (not os.path.exists(filename) or overwrite):
        if (with_points):
            res = decoder(points.unsqueeze(0), points.unsqueeze(0),None, latent=latent, only_decoder_forward=True, only_encoder_forward=False)
            pnts_val = res
        ...
        for key,val in z_func.items():
            ...
            layout = go.Layout(title= go.layout.Title(text="epoch : {0} <br> input filename:{1}".format(epoch, shapefile)), width=1200, height=1200, scene=dict(camera=dict(up=dict(x=0, y=1, z=0),center=dict(x=0, y=0.0, z=0),eye=dict(x=0, y=0.6, z=0.9)),xaxis=dict(range=[-2, 2], autorange=False),
                                                                                                            ...
                                                                                                            aspectratio=dict(x=1, y=1, z=1)))
            if (with_points):
                trace_pnts = get_scatter_trace(points[:, -3:].cpu(),name="input", caption=caption)
...
def get_surface_trace(points, decoder, latent, resolution, mc_value, is_uniform, verbose, save_ply, grid_boundary=2.0,is_3d=True,name='',z_func=None):
    ...
    for pnts in tqdm(torch.split(grid['grid_points'], 10000, dim=0)):
        ...
        #     v = decoder(pnts, points,sample_nonmnfld=None, latent=latent.repeat(len(decoder.device_ids),1), only_encoder_forward=False, only_decoder_forward=True)
        # else:
        v = decoder(pnts, None,None, latent=latent, only_encoder_forward=False, only_decoder_forward=True)
        if type(v) is tuple:
            v = v[0]
...
def get_grid_uniform(points,resolution,grid_boundary,is_3d):
    if is_3d:
        ...
        y = np.arange(-1, 1 + length / (x.shape[0] - 1), length / (x.shape[0] - 1) )
